...
export default function CognitiveReframing() {
  ...
  const handleSave = async () => {
    if (!user) {
      const pendingThoughtRecord = {
        situation,
        emotion,
        automaticThought: thought,
        rationalThought: counterThought,
        date: new Date().toISOString()
      };
      sessionStorage.setItem('pendingThoughtRecord', JSON.stringify(pendingThoughtRecord));
      navigate('/auth');
      return;
    }

    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/api/thought-records`, {
        situation,
        emotion,
        automaticThought: thought,
        rationalThought: counterThought,
    ...
    <div className="min-h-screen bg-slate-50 pt-28 pb-20">
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-6">
          ...
          <button onClick={handleSave} className="w-full bg-[#e87a5d] hover:bg-[#d6694c] text-white font-medium py-3 rounded-md transition-colors shadow-sm">
            Save Thought Record
          </button>
        </div>
      </div>
