    ...
    <div className="min-h-screen bg-slate-50 py-20 pt-32">
      <div className="container mx-auto px-4">
        <div className="max-w-md mx-auto bg-white/60 backdrop-blur-lg rounded-2xl p-8 shadow-xl border border-white/40 relative">
          ...
          <div className="absolute -top-4 -right-4 bg-slate-100 text-slate-600 text-xs font-bold px-3 py-1 rounded-full border border-slate-200 uppercase tracking-wider shadow-sm">
            Staff Portal
          </div>
          ...
          {step === 'login' && (
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                ...
                <input type="text" value={identifier} onChange={(e) => setIdentifier(e.target.value)} className="w-full h-12 rounded-xl border border-slate-200 px-4 focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Password</label>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full h-12 rounded-xl border border-slate-200 px-4 focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white" required />
              </div>
              <div className="flex justify-end mt-1 mb-2">
                <button type="button" onClick={() => { setStep('forgot'); setError(''); setSuccessMessage(''); }} className="text-sm font-medium text-slate-500 hover:text-slate-800 transition-colors">Forgot password?</button>
              </div>
              <button type="submit" disabled={loading} className="w-full bg-teal-600 hover:bg-teal-700 text-white h-12 rounded-xl text-sm font-medium shadow-md hover:shadow-lg transition-all disabled:opacity-50 flex justify-center items-center">
          ...
          {step === 'forgot' && (
            <form onSubmit={handleForgotSend} className="space-y-4">
              ...
              <button type="submit" disabled={loading} className="w-full bg-teal-600 hover:bg-teal-700 text-white h-12 rounded-xl text-sm font-medium shadow-md transition-all disabled:opacity-50 flex justify-center items-center">
          ...
          {step === 'verify' && (
            <form onSubmit={(e) => { e.preventDefault(); if (otp.length === 6) setStep('reset'); else setError('Enter a 6-digit code'); }} className="space-y-4">
              <div>
                ...
                <input type="text" maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))} className="w-full h-12 rounded-xl border border-slate-200 px-4 text-center text-2xl tracking-[0.5em] font-mono focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white" required autoFocus />
              </div>
              <button type="submit" className="w-full bg-teal-600 hover:bg-teal-700 text-white h-12 rounded-xl text-sm font-medium shadow-md transition-all">Verify Code</button>
