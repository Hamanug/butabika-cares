...
export default function Messages() {
    ...
    const fetchContacts = async () => {
      try {
        const endpoint = user?.role === 'therapist' 
          ? 'http://localhost:3000/api/appointments/my-sessions' 
          : 'http://localhost:3000/api/therapists/active';
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/messages/${activeChat.id}`, { withCredentials: true });
